...
def ensure_session(db: Session, *, session_id: str | None = None, user_id: str) -> ChatSession:
    if session_id:
        existing = db.get(ChatSession, session_id)
        if existing:
            if existing.user_id != user_id:
                raise SessionAccessError("Session does not belong to current user")
            return existing
...
def list_session_messages(db: Session, *, session_id: str, user_id: str) -> list[dict]:
    session = db.get(ChatSession, session_id)

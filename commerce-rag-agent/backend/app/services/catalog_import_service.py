...
from app.services.taxonomy import normalize_product_taxonomy
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        try:
            if product_id in seen_product_ids:
                raise ValueError("duplicate product id in import file")
            if product_id:
...
def normalize_catalog_row(row: dict[str, str]) -> dict[str, Any]:
    ...
    category, subcategory = normalize_product_taxonomy(
        row["category"].strip(),
        (row.get("subcategory") or row.get("sub_category") or "").strip(),
...
def upsert_product(
    ...
    *,
    ...
    add_tags(db, row["id"], "category", [row["category"]])
    add_tags(db, row["id"], "subcategory", [row["subcategory"]])

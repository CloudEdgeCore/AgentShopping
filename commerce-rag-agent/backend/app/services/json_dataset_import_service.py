...
from app.services.taxonomy import load_taxonomy, normalize_product_taxonomy
...
def upsert_dataset_product(
    ...
    *,
    ...
    category, sub_category = normalize_product_taxonomy(
        required_text(payload, "category"),
        str(payload.get("sub_category") or payload.get("subcategory") or "").strip(),
...
def extract_keywords(text: str) -> list[str]:
    return [keyword for keyword in _dataset_keywords() if keyword in text]


def _dataset_keywords() -> list[str]:
    keywords: list[str] = []

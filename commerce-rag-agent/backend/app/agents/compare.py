...
from app.agents.shopping_guide import build_retrieved_items, product_to_card, products_to_cards
...
def compare_node(db: Session, *, orchestrator: RetrievalOrchestrator | None = None):
    def node(state: dict) -> dict:
        ...
        if len(products) < 2 and orchestrator is not None:
            try:
                ...
                products = retrieval_result.products[:4]
        ...
        if len(products) < 2:
            return {
                **state,
                "answer": str(rule_section("compare", "need_two_products_text") or "可以对比，但我需要至少两个明确商品。"),
                "product_cards": [],
                "retrieved_items": [],
        ...
        products = products[:4]
        cards = (
            products_to_cards(products, state.get("memory", {}), retrieval_result)
            if retrieval_result is not None
            else [product_to_card(product, state.get("memory", {}), rank) for rank, product in enumerate(products, start=1)]
        )
        ...
        return {
            **state,
            ...
            "retrieved_items": (
                ...
                if retrieval_result is not None
                else [{"product_id": product.id, "title": product.title} for product in products]
            ),
            "product_cards": cards,
...
def _matched_keywords(product, keywords: list[str], docs: list[dict]) -> list[str]:
    haystack = " ".join(
        [product.title, product.category, product.subcategory or "", product.description, " ".join(doc.get("text", "") for doc in docs)]
    )
...
def _best_for(product, matched_keywords: list[str]) -> str:
    ...
    return f"适合看重{product.subcategory or product.category}基础体验的用户"

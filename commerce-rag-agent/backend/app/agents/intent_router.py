...
def classify_intent(text: str) -> IntentResult:
    ...
    if nlu.is_purchase_intent(normalized):
        return IntentResult(intent="purchase_help", confidence=0.9, constraints=constraints)

    if _is_complex_product_reasoning(normalized):
        return IntentResult(intent="compare", confidence=0.87, constraints=constraints)
    ...
    if _contains_any(normalized, ["这款", "这两个", "这两款", "这几个", "这个", "有没有", "换一个", "更轻", "便宜点", "评价", "评论", "口碑"]):
...
def _extract_audience(text: str) -> str | None:
    audience_aliases = rule_dict("nlu", "audience_aliases")
    for audience, aliases in audience_aliases.items():
        if isinstance(aliases, list) and _contains_any(text, [str(alias) for alias in aliases]):
            return str(audience)
    return None
...
def _is_complex_product_reasoning(text: str) -> bool:
    return _contains_any(
        text,
        [
            "\u8be6\u7ec6\u5206\u6790",
